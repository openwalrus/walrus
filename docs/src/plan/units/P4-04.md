...
Implement DirectRunner — a struct that wraps `Runtime<InMemory>` for running
...
- DirectRunner owns a `Runtime<InMemory>` and provides send/stream methods
  matching the Runtime API. Uses InMemory since CLI direct mode has no
  config-driven memory backend selection.
