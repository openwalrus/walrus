...
- SqliteMemory holds `rusqlite::Connection` in a `Mutex` (Connection is not Send).
- Optional `Arc<dyn Embedder>` for future vector search.
...
- `SqliteMemory::with_embedder(self, Arc<dyn Embedder>) -> Self`
