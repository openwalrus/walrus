...
use wcore::model::{FunctionCall, Message, Request, Tool, ToolCall};
...
fn resolve_returns_registered_tools() {
    let mut rt = Runtime::<()>::new(Request::default(), (), InMemory::new());
...
fn resolve_skips_unknown() {
    let rt = Runtime::<()>::new(Request::default(), (), InMemory::new());
...
fn context_limit_default() {
    let rt = Runtime::<()>::new(Request::default(), (), InMemory::new());
    // ()'s Model impl returns 0 for context_limit.
    assert_eq!(rt.context_limit("any-model"), 0);
}
...
async fn system_prompt_includes_memory() {
    ...
    let mut rt = Runtime::<()>::new(Request::default(), (), memory);
...
fn skill_body_injected() {
    ...
    let mut rt = Runtime::<()>::new(Request::default(), (), InMemory::new()).with_skills(registry);
    rt.add_agent(
        Agent::new("dev")
